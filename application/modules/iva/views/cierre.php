...
<!--    <td><?php echo $factura->tipcomp, " ", $factura->letra, $factura->puesto, "-", $factura->numero;?></td> -->
    ...
    <td align="center">
      <?php if($factura->suma==0):?>
        No Suma
      <?php else :?>
          (-)<?php echo form_radio($factura->id, 0 , ($factura->ivapass==0)?true:false,'id="radio_'.$factura->id.'-"');?>
          (+)<?php echo form_radio($factura->id, 1 , ($factura->ivapass==1)?true:false,'id="radio_'.$factura->id.'+"');?>
      <?php endif;?>
    </td>
  </tr>
<?php endforeach;?>
